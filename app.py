...
def delete_patient_record(db, delete_option, delete_value):
  ...
  # Delete appointments first (if any)
  cursor.execute(f"DELETE FROM appointments WHERE patient_id = (SELECT id FROM patients WHERE {delete_option} = %s)", (delete_value,))
  db.commit()

  # Then delete the patient record
  cursor.execute("USE userdb")
  delete_patient_query = f"DELETE FROM patients WHERE {delete_option} = %s"
  cursor.execute(delete_patient_query, (delete_value,))
  db.commit()
  st.write("Patient record deleted successfully.")
...
def main():
    ...
    menu = ["Home","Add patient Record","Show patient Records", "Search and Edit Patient","Delete Patient Records",
    ...
    elif options == "Delete Patient Records":
